...
app.def('consumeBody', function(req, contentLength, maxBufferedContentLength, error) {
  return function consumeBody(push) {
    ...
    req.on('data', function(data) {
      ...
      if (wait) pauseConsumption(req, wait, result)
    })
...
function pauseConsumption(req, wait, result) {
  let f = wait.__to_go_future()

  if (f.ready) {
    if (f.error) result.done(f.error)
    return
  }
...
app.def('error', function() {
  return function error(status, msg, options) {
    if (arguments.length < 3) {
...
app.def('beforeRequestProcessing', {pre: []})


app.def('requestProcessing', function*(evaluate, req, res, onHttpException, sendStatus, beforeRequestProcessing$, route$, sendResult$) {
  try {
    yield beforeRequestProcessing$()

    let route = yield route$()
    if (!route) return sendStatus(404)
...
app.def('serve', function*(requestProcessing$, onServerError) {
  try {
