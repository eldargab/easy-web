...
describe('http.send', function () {
  ...
  describe('Should send body', function (done) {
    ...
    it('stream', function (done) {
      supertest(function (req, res) {
        var stream = fs.createReadStream(join(__dirname, 'fixtures/hello.html'))
        var o = new http.Response().send(stream).type('text/html')
        stream.on('error', done)
        http.send(req, res, o, function () {
          stream.destroy()
        })
      })
      .get('/')
  ...
  describe('Cookies', function () {
    ...
    it.skip('Should support setting of multiple cookies', function (done) {
    ...
    it('Should preserve other headers when cookies were set', function (done) {
      o.set('foo', 'bar').cookie('baz', 'qux')
      request().end(function (err, res) {
        if (err) return done(err)
        res.headers.should.have.property('foo').equal('bar')
        done()
      })
    })
  })
