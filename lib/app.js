...
App.dispatch = function(req, res) {
  ...
  this.set('req', new Request(req))

  this.eval('dispatch', function(err) {
...
App.useDispatch = function() {
  if (this.defined('dispatch')) return
  this.def('dispatch', function(req, done) {
    var task = this.router().dispatch(req.path, req)
    this.eval(task, done)
    // more correct way is to take `eval` dependency
    // and do eval(task, done)
    // but that will slow down hello world server up to 10 %
    // if we move this dispatch logic right to app.dispatch()
    // we'll receive additional 10 % boost
    // That's cost of our tasks (and scopes in general)
  })
}
